...
//                         const res = await database.executeSql('INSERT OR IGNORE INTO books (number, name) VALUES (?, ?)', [book_id, book_name]);
//                         const book_ids = res[0].insertId || (await database.executeSql('SELECT id FROM books WHERE name = ?', [book_name]))[0].rows.item(0).id;
//                         bookMap.set(book_name, book_ids);
//                         console.log('inserted book', book_ids);
//                     }

//                     const bookId = bookMap.get(book_name);
...
// export const insertBookmark = async ({ color, verse_id }: any) => {
//     console.log('insertBookmark', color, verse_id);
//     const database = await SQLite.openDatabase({
//         name: DATABASE_NAME,
//         location: 'default',
//     });
//     try {
//         const [results] = await database.executeSql(`INSERT INTO ${TABLE_BOOKMARKS} (verse_id, color) VALUES (?, ?);`, [verse_id, color]);
//         console.log(`Bookmark inserted: ${verse_id}, ID: ${results.insertId}`);
//         await database.close();
//         return results;
//     } catch (error) {
//         console.error('Error inserting search history:', error);
...
//     }
// };
// export const getVersesByChapter = async (chapterNumber: number) => {
//     return new Promise(async (resolve, reject) => {
//         if (!chapterNumber || chapterNumber === undefined) {
//             reject('Book name and chapter number are required.');
//         }
//         const database = await SQLite.openDatabase({
//             name: DATABASE_NAME,
//             location: 'default',
...
//                 `SELECT * FROM ${TABLE_VERSES} WHERE chapter_id = ? ORDER BY number;`,
//                 [chapterNumber]
//             );
//             const verses = [];
//             for (let i = 0; i < results.rows.length; i++) {
//                 verses.push(results.rows.item(i));
//             }
//             console.log(`Retrieved ${verses.length} verses chapter ${chapterNumber}.`);
//             resolve(verses);
//         } catch (error) {
//             console.error(`Error getting verses for ${chapterNumber}:`, error);
//             reject(error);
//         } finally {
//             await database.close();
//         }
...
//         });
//         try {
//             const [results] = await database.executeSql(`SELECT * FROM ${TABLE_VERSES} ORDER BY RANDOM() LIMIT 1;`);
//             const verse = results.rows.item(0);
//             await database.close();
//             resolve(verse);
//         } catch (error) {
...
// export const saveVerseToNotifications = async (verse: any) => {
//     return new Promise(async (resolve, reject) => {
//         const database = await SQLite.openDatabase({
//             name: DATABASE_NAME,
//             location: 'default',
//         });
//         try {
//             const [results] = await database.executeSql(`INSERT INTO ${TABLE_NOTIFICATIONS} (verse_id) VALUES (?);`, [verse.id]);
//             await database.close();
//             resolve(results);
//         } catch (error) {
//             console.error('Error saving verse to notifications:', error);
//             await database.close();
//             reject(error);
//         }
//     });
...
//     console.log('deleteBookmarkById', id);
//     return new Promise(async (resolve, reject) => {
//         const database = await SQLite.openDatabase({
//             name: DATABASE_NAME,
//             location: 'default',
//         });
//         try {
//             const [results] = await database.executeSql(`DELETE FROM ${TABLE_BOOKMARKS} WHERE id = ?;`, [id]);
//             await database.close();
//             resolve(results);
//         } catch (error) {
